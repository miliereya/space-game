...
export class Scene {
	...
	constructor(props: SceneProps = {}) {
		...
		if (props.metrics) {
			this.metrics = new Metrics(this.TWorld, this.cannon, props.metrics)
		}
		// Event listeners
		this.bindEvents()
	...
	private bindControlPanelEvents() {
		document.getElementById('on')?.addEventListener('click', () => {
			...
			if (!this.isRocketLaunched) {
				...
				this.cannon.addBody(this.rocket.body)
			}
		})
		...
		document.getElementById('stage2')?.addEventListener('click', () => {
			this.rocket.startSecondStage(this.cannon, this.TWorld)
		})

		document.getElementById('stage3')?.addEventListener('click', () => {
			this.rocket.startThirdStage(this.cannon, this.TWorld)
		})
